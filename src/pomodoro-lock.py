...
class PomodoroLock:
    ...
    def check_user_activity(self):
        ...
        # Update last activity time if there's any keyboard/mouse activity
        try:
            # Get current pointer position
            pointer = root.query_pointer()
            if pointer:
                self.last_activity = current_time
                logging.debug("Activity detected: mouse movement")
        except Exception as e:
            logging.error(f"Error checking pointer: {e}")
        ...
        for proc in psutil.process_iter(['pid', 'name']):
            try:
                if proc.info['name'] in ['pulseaudio', 'pipewire']:
                    sound_active = True
                    self.last_activity = current_time
                    logging.debug("Activity detected: sound playing")
                    break
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                pass

        # Log inactivity if detected
        if (current_time - self.last_activity) > self.inactivity_threshold:
            elapsed_time = (current_time - self.last_activity) / 60
            logging.info("Inactivity detected for more than %d minutes. Elapsed time: %.2f minutes. Resetting work timer.", 
                        self.config['inactivity_threshold_minutes'], elapsed_time)
            self.remaining = self.work_time
            self.notification_sent = False
            self.state = 'work'
            self.timer.update_timer(self.remaining, mode='work')
